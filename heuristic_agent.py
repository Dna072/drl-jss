...
def get_heuristic_action(env: FactoryEnv):
    ...
    p_jobs_ptd = FactoryEnv.get_actual_process_time_to_deadline_ratio(
        pj
    ...
    job_indices = indices_of_extreme_n(p_jobs_ptd, find_minimum=False)
    ...
    if uc_job_index == -1 or max(p_jobs_ptd) > max(uc_jobs_ptd):
        ...
        for job_idx in job_indices:
            for idx, m in enumerate(am):
                if m.can_perform_job(pj[job_idx]) and m.is_available():
                    if m.get_pending_tray_capacity() < pj[job_idx].get_tray_capacity():
                    ...
                    if (
                        ...
                        != pj[job_idx].get_next_pending_recipe().get_factory_id()
                    ):
...
if __name__ == "__main__":
    ...
    for e in range(episodes):
        ...
        while (
            ...
            print(
                f"Pending jobs actual ptd: {FactoryEnv.get_actual_process_time_to_deadline_ratio(env.get_pending_jobs())} "
            )
            print(
            ...
            if te:
                ...
                avg_tardiness_of_late_jobs_per_episode.append(
                    i["AVG_TARDINESS_OF_LATE_JOBS"]
                )
                current_step = 0
                returns = []
                obs, info = env.reset()
    ...
    print(
        f"Avg jobs completed on time: {sum(jobs_completed_on_time) / len(jobs_completed_on_time)}"
    )
    print(
        f"Avg jobs completed late: {sum(jobs_completed_late) / len(jobs_completed_late)}"
    )
    print(
        f"Avg tardiness of late jobs over episodes: "
