...
DQN_AGENT_PATH = "files/trainedAgents/dqn_seco_3m_3r_0.99g_128_128_64_ma_obs_x6_60200000"
...
HEURISTIC_PATH = SAVE_PATH + f"heuristic_data_seco_{N_MACHINES}m_{N_RECIPES}r_{JOBS_BUFFER_SIZE}b_seco_recipes_ma_obs_u_" + str(
    N_EPISODES) + ".pkl"
# HEURISTIC_PATH = 'files/data/heuristic_data_seco_2m_2r_3b_step_ptd_1000.pkl'
if not os.path.exists(HEURISTIC_PATH):
    heuristic_rewards, heuristic_tardiness, heuristic_jot, heuristic_jnot = episodic_heuristic_agent(
...
EDD_PATH = SAVE_PATH + f"edd_data_seco_{N_MACHINES}m_{N_RECIPES}r_{JOBS_BUFFER_SIZE}b_seco_recipes_j_q_ma_obs_u_" + str(
    N_EPISODES) + ".pkl"
...
FIFO_PATH = SAVE_PATH + f"fifo_data_seco_{N_MACHINES}m_{N_RECIPES}r_{JOBS_BUFFER_SIZE}b_seco_recipes_j_q_ma_obs_u_" + str(
    N_EPISODES) + ".pkl"
...
DQN_PATH = SAVE_PATH + f"dqn_data_seco_{N_MACHINES}m_{N_RECIPES}r_{JOBS_BUFFER_SIZE}b_{GAMMA}g_norm_recipes_ma_obs_pure_60M_x6_" + str(
    N_EPISODES) + ".pkl"
...
A2C_PATH = SAVE_PATH + f"a2c_data_seco_{N_MACHINES}m_{N_RECIPES}r_{JOBS_BUFFER_SIZE}b_seco_recipes_ma_obs_60M_u_" + str(
    N_EPISODES) + ".pkl"
...
heuristic_tardiness_performance = [a / (b + c) if b + c > 0 else 0 for a, b, c in
                                   zip(heuristic_tardiness, heuristic_jot, heuristic_jnot)]
...
plt.bar(heuristic_positions, heuristic_tardiness_performance, width=bar_width, label='heuristic Tardiness',
        color='#2ca02c')
