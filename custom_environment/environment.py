...
class FactoryEnv(gym.Env):
    ...
    _MACHINE_IS_AVAILABLE_STR: str = "machine_is_available"
    ...
    def __init__(
        ...
        # 0 - Machine not available
        # 1 - Machine available
        machine_is_available_space: gym.spaces.Box = gym.spaces.Box(
            low=0, high=1, shape=(len(self._machines),), dtype=np.float64
        )
        ...
        self.observation_space: gym.spaces.Dict = gym.spaces.Dict(
            {
                ...
                self._MACHINE_IS_AVAILABLE_STR: machine_is_available_space,
    ...
    def get_capacity_obs(self) -> dict[str, np.ndarray[float]]:
        ...
        return {
            ...
            self._MACHINE_IS_AVAILABLE_STR: machine_is_available,
    ...
    def update_buffer(self, removed_job_idx: int = None):
        ...
        if len(self._pending_jobs) < self._BUFFER_LEN:
            ...
            new_job = create_job(
                ...
                factory_id="J" + str(int(self.factory_time) % self._BUFFER_LEN),
            ...
            if removed_job_idx is not None:
                self._pending_jobs.insert(removed_job_idx, job_to_move)
            else:
